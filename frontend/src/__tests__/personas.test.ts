...
/** personas.ts의 평면 프리셋을 엔진 UserProfile로 (App.applyPersona와 동일 매핑). */
...
describe("데모 페르소나 (와우모먼트)", () => {
  it("세 페르소나 모두 결과(워터폴 또는 긴급)를 만든다 — 빈 화면 없음", () => {
    for (const p of PERSONAS) {
      const rec = recommend(toProfile(p.id, asOf), ruleSet);
      expect(rec.waterfall.length + rec.urgent.length).toBeGreaterThan(0);
    }
  });
  ...
  it("세 페르소나의 결과 구성(긴급+워터폴 productId 집합)이 서로 다르다 — 시연 차별성", () => {
    const sig = (id: string) => {
      const rec = recommend(toProfile(id, asOf), ruleSet);
      return [
        ...rec.urgent.map((u) => `U:${u.productId}`),
