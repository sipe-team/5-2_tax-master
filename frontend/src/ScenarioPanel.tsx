...
function JobResultChip({ job, onPick }: { job: JobChip; onPick: (j: JobChip) => void }) {
  return (
    <button
      type="button"
      onClick={() => onPick(job)}
      className="rounded-xl border border-line px-3 py-2 text-left text-[12px] outline-none transition-colors hover:border-gold focus-visible:border-gold"
    >
    ...
    <section className="mb-7 rounded-2xl bg-surface p-5 ring-1 ring-line">
      ...
      <div className="mt-5">
          ...
          <div className="mb-5 border-b border-line pb-5">
            <div className="flex items-end gap-2">
              <label className="flex flex-1 flex-col gap-1">
                <span className="text-[11px] tracking-wide text-muted">관심 공고 검색 (선택)</span>
                <input
                  className="w-full bg-transparent border-b border-line pb-1 text-[15px] text-ink outline-none focus:border-gold placeholder:text-locked"
                  value={keyword}
              ...
              <button
                ...
                onClick={runSearch}
                disabled={searching}
                className="rounded-lg border border-gold/50 px-3 py-1.5 text-[13px] text-gold outline-none hover:bg-gold/10 disabled:opacity-50"
              >
                {searching ? "검색중…" : "검색"}
              </button>
            </div>
            ...
            {picked && (
              <div className="mt-3 rounded-xl bg-paper/50 px-3 py-2 text-[12px] ring-1 ring-line">
          ...
          <label className="flex flex-col gap-1">
            ...
            <span className="flex items-baseline gap-1.5">
              <input
                type="text"
                inputMode="numeric"
                className="w-28 bg-transparent border-b border-line pb-1 font-display text-lg tnum text-ink outline-none focus:border-gold"
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2 text-[13px]">
            ...
            <select
              className="bg-transparent border-b border-line pb-1 text-[14px] text-ink outline-none focus:border-gold"
          ...
          <div className="mt-5 grid grid-cols-2 gap-3">
            <div className="rounded-xl bg-paper/50 p-3 ring-1 ring-line">
              <div className="text-[11px] text-muted">한계세율</div>
              <div className="mt-1 font-display tnum text-lg">
